...
if (!class_exists('copy_move_comments')) {
    ...
    class copy_move_comments {
        ...
        function copy_move_menu() { // Dispaly Seperate Menu
            ...
            add_submenu_page('copy-move', 'Setting', 'Setting', 'administrator', 'copy-move-setting', array($this, 'copy_move_setting_func'));
            add_action('admin_init', array($this, 'register_copy_move__suggest_settings')); // Register all post type
            //add_action( 'admin_init', 'copy_move_suggest_setting' );
        }
        ...
        function copy_move_setting_func() {
            ...
            if (isset($_REQUEST['settings-updated']) == 'true') {
                ?>
                <div class="updated notice notice-success is-dismissible" id="message"><p>Setting updated.</p><button class="notice-dismiss" type="button"><span class="screen-reader-text">Dismiss this notice.</span></button></div>
                <?php
            }
            ?>
                    ...
                    <td>
                        <fieldset>
                            <?php {
                                ...
                                ?>
                                <input type="checkbox" id="all_private_post_type" name="all_private_post_type" <?php if (isset($checked)) echo $checked; ?> />
                                <label for="all_private_post_type">Include Private Posts?</label><br>
                                <?php
                                $checked = '';
                            }
                            ?>
                ...
                <form id="copy_move_form" action="admin-post.php" method="post">
                    ...
                    <div class="tablenav top">
                        ...
                        <div class="alignleft actions" id="comment_history">
                            <big style="float: left; margin-top: 5px; margin-right: 7px;"><?php _e("Select Comment Type"); ?>:</big>
                            <select id="comment_type" name="comment_type">
                                <option value="0">Select Comment Type</option>
                                <!--<option value="1">Single</option>
                                <option value="2">With Replies</option>-->
                            </select>
                            <span id="bc_loader1" style="display: none;"><img src="<?php echo plugins_url('ajax-loader.gif', __FILE__); ?>" alt=""></span>
                        </div>
...
function get_post_comments_callback() {
    ...
    <table class="wp-list-table widefat fixed posts">
        <thead>
            <tr>
                <th style="" class="manage-column column-cb check-column" scope="col"><input type="checkbox" value="0" name="move_comment_id1[]" onchange="checkAll(this);"></th>
                ...
                <th style="" class="manage-column column-date sortable asc" scope="col">In Reply To</th>
        ...
        <tbody>
            <?php
            if (isset($comment_type) && $comment_type == '0') {
                ?>
                <tr>
                ...
                if (!empty($get_comments)) {
                    for ($k = 0; $k < count($get_comments); $k++) {
                        $get_comment_ids[] = $get_comments[$k]->comment_id;
                    }
                    $ids = $get_comment_ids;
                    do {
                        $tem = $ids;
                    ...
                    do {
                        ...
                        for ($i = 0; $i < count($get_comment_ids_1); $i++) {
                            if (in_array($get_comment_ids_1[$i], $printed))
                                continue;
                            else {
                                ...
                                <tr class="<?php echo $cls; ?>" id="<?php echo $c; ?>">
                                    <?php
                                    if (isset($comment_type) && $comment_type == '2') {
                                        if (!in_array($get_comment_ids_1[$i], $ids)) {
                                            ?>
                                            <td></td>
                                            <?php
                                        } else {
                                    ...
                                    <td>
                                        <?php
                                        if (!in_array($get_comment_ids_1[$i], $ids)) {
                                            echo str_repeat(" - ", count($temp));
                                        }
                                        echo $get_comment->comment_content;
                                        ?></td>
                                    <td>
                                        <?php
                                        if ($get_comment->comment_parent != '0') {
                                            $child_comment = get_comment($get_comment->comment_parent);
                                            echo $autho_name = $child_comment->comment_author;
                                        } else {
                                            echo "-";
                                        }
                                        ?>
                                ...
                                if (isset($ary) && !empty($ary)) {
                                    if (count($ary[$get_comment_ids_1[$i]]) > 0) {
                        ...
                        if ($i == count($get_comment_ids_1))
                            @$get_comment_ids = array_pop($temp);
                    }while (count($get_comment_ids) > 0);
                }
                else {
                    ?>
                    <tr>
...
function prefix_admin_action_move() {
    ...
    $get_comment_type = $_REQUEST['comment_type'];
    ...
    if (isset($get_comment_type) && $get_comment_type == '1') {
        ...
        do {
            ...
            for ($i = 0; $i < count($tem); $i++) {
                $s = "select * from $wpdb->comments where comment_parent = '" . $tem[$i] . "' and comment_Approved != 'trash'";
                $data = $wpdb->get_results($wpdb->prepare("", $id));
                $res = $wpdb->get_results($s);
                if ($res) {
                    for ($j = 0; $j < count($res); $j++) {
                        $ary[$tem[$i]][] = $res[$j]->comment_ID;
...
function error_message() {
    ...
    if (isset($_REQUEST['error']) && $_REQUEST['error'] == 1) {
        ?>
        <div class="error">
            <p><?php _e('Please select action OR select atleast one comment to copy/move.'); ?></p>
        </div>
        <?php
    }
}
...
function copy_and_move_wp_admin_enqueue_scripts() {
    ...
    // styles
    wp_register_style('cm-select2-css', plugins_url('css/select2.min.css', __FILE__));

    // scripts
    wp_register_script('cm-select2-script', plugins_url('js/select2.min.js', __FILE__));
}
?>