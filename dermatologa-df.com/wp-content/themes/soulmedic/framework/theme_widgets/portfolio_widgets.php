			 ...
			 while(have_posts()):
				...
				echo "<li>";
					if(1 == $_enabled_image):
						 ...
						 if(  array_key_exists('items_id',$portfolio_settings) ){
							 $items_id_exists =  array_filter($portfolio_settings['items_id']);
						 ...
						 if( has_post_thumbnail() ):
							...
							$image = $thumb[0];
						 elseif( array_key_exists("items_name",$portfolio_settings) ):
						 	$item =  $portfolio_settings['items_name'][0];
							
							if( "video" === $item ) {
								$items = array_diff( $portfolio_settings['items_name'] , array("video") );
								if( !empty($items) ) {
									if( $items_id_exists ) {
										$id = $portfolio_settings['items_id'][key($items)];
										$thumb = wp_get_attachment_image_src( $id , 'thumbnail',true);
										$image = $thumb[0];
									} else {
										$image = $portfolio_settings['items'][key($items)];
									}
								}
							} else {
								if( $items_id_exists ) {
									$id = $portfolio_settings['items_id'][0];
									$thumb = wp_get_attachment_image_src( $id , 'thumbnail',true);
									$image = $thumb[0];
								} else {
