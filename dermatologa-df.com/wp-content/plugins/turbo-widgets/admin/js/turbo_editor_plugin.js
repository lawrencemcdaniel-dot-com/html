(function() {
    tinymce.create('tinymce.plugins.TurboPlugin', {
        init : function(ed, url) {
            ...
            // TODO - Make Premium branch here - show PREMIUM buy in, if non-premium
            ed.on('DblClick',function(e) {
                var cls  = e.target.className.indexOf('wp-turboplugin');
                if ( e.target.nodeName == 'IMG' && e.target.className.indexOf('turbo_widget') > -1 ) {
                    var tw_attrs = e.target.attributes['data-sh-attr'].value;
                    tb_show( 'Modify existing Turbo Widget', 'admin-ajax.php?action=turbo_editor_dialog&tw_attrs=' + escape(tw_attrs) );
        ...
        _do_turbo : function(co) {
            return co.replace(/\[turbo_widget([^\]]*)\]/g, function(a,b){
                ...
                b = b.replace(/&amp;id_base=/g, '&id_base=');
                var queryDict = {};
                b.substr(1).split("&").forEach(function(item) {queryDict[item.split("=")[0]] = item.split("=")[1]});
                return '<img src="../wp-content/plugins/turbo-widgets/images/tw.png" class="turbo_widget mceItem" data-sh-attr="turbo_widget'+tinymce.DOM.encode(b)+'" title="Turbo Widget: ' + queryDict['widget-prefix'] + '. Double click to edit "/>';
