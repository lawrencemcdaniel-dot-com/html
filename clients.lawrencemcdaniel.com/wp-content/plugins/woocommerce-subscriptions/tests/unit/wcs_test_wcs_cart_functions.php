...
class WCS_Cart_Functions_Unit_Tests extends WCS_Unit_Test_Case {
	...
	*/
	public function provider_test_wcs_cart_totals_shipping_method() {
		...
		$cart = $this->getMockBuilder( 'WC_Cart' )->getMock();
		return array(
			array( $cart, new WC_Shipping_Flat_Rate(), 'flat_rate' ),
			array( $cart, new WC_Shipping_Free_Shipping(), 'free_shipping' ),
			array( $cart, new WC_Shipping_Local_Pickup(), 'local_pickup' ),
			array( $cart, null, ': ' ),
		);
	}
	...
	*/
	public function test_wcs_cart_totals_shipping_method( $cart, $shipping_method, $expected ) {

		// Standard Shipping Methods
		if ( ! empty( $shipping_method ) ) {
			$shipping_method->label = $shipping_method->id;
		...
		$this->assertContains( $expected, wcs_cart_totals_shipping_method( $shipping_method, $cart ) );
	}
