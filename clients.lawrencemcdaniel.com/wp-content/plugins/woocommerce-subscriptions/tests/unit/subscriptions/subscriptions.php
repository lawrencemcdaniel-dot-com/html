...
class WC_Subscription_Test extends WCS_Unit_Test_Case {
	...
	public function test_calculate_end_of_prepaid_term_date() {
		...
		$subscription->update_dates( array(
			'date_created' => gmdate( 'Y-m-d H:i:s', strtotime( '-2 weeks', $now ) ),
	...
	public function test_get_date_not_gmt() {
		...
		$this->assertEquals( get_date_from_gmt( $start_date ), $subscription->get_date( 'date_created', 'site' ) );
		$this->assertEquals( get_date_from_gmt( $start_date ), $subscription->get_date( 'date_created', 'other' ) );
	}
	...
	public function test_get_date_gmt() {
		...
		$this->assertEquals( $expected_result, $subscription->get_date( 'date_created', 'gmt' ) );
		$this->assertEquals( $expected_result, $subscription->get_date( 'date_created' ) );
	}
	...
	public function test_calculate_next_payment_date_active() {
		...
		$subscription->update_dates( array(
			'date_created' => '2000-12-01 00:00:00',
			'trial_end'    => 0,
	 ...
	 *
	 * @expectedDeprecated WC_Subscription::delete_date
	 * @since 2.0
	 */
	...
	public function test_get_related_orders() {
		...
		$this->assertEquals( 3, count( $related_order_ids ) );
		...
		foreach ( WCS_Helper_Subscription::create_subscriptions() as $status => $subscription ) {
			...
			if ( in_array( $status, $expected_to_pass ) ) {
				...
				try {
					...
					$subscription->update_dates( array( 'date_created' => $start_date ) );
	...
	public function update_dates_data() {
		return array(
			...
			array(
				array(
					'date_created' => '2015-01-08 10:10:01',
			...
			array(
				array( // dates to set on subscription before calling update_dates
					'date_created'            => '2015-05-08 12:49:32',
	...
	public function update_dates_data_exceptions() {
		return array(
			...
			array(
				...
				array( // Expected
					'exception'    => 'Exception',
					'message'      => 'The end date must occur after the last payment date. The end date must occur after the start date.'
				)
			),
			array(
				array( // dates to set on subscription before calling update_dates
					'end'          => '2016-01-17 12:47:32',
					'date_created' => '2015-03-08 12:47:32',
				),
				array( // update_dates() input array
				...
				array( // Expected
					'exception'    => 'Exception',
					'message'      => 'The end date must occur after the next payment date. The end date must occur after the start date.'
				)
			),
			array(
				array( // dates to set on subscription before calling update_dates
				...
				array( // update_dates() input array
					...
					'date_created' => '2015-05-08 12:48:32',
				),
				array( // Expected
					'exception' => 'Exception',
					'message'   => 'The end date must occur after the last payment date. The end date must occur after the next payment date. The end date must occur after the trial end date. The end date must occur after the start date.'
				)
			),
			array(
				array(
					'date_created' => '2015-05-08 12:50:32',
				...
				array(
					'exception' => 'Exception',
					'message'   => 'The trial_end date must occur after the start date. The end date must occur after the next payment date. The end date must occur after the start date.'
				)
			),
		);
	}
