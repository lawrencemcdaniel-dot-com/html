...
function plugin_head_styles_scripts() {
	...
	wp_enqueue_script('meanmenu-script', IAMD_FW_URL.'js/public/jquery.meanmenu.js',array(),false,true);
	...
	if( is_page_template('tpl-portfolio.php') || is_page_template('tpl-blog.php')):
...
function dttheme_appearance_css() {
	...
	if (empty($disable_menu)) :
		...
		if (!empty($font) || (!empty($primary_color) and $primary_color != "#") || !empty($size)) :
			...
			$output .= "#main-menu ul.menu li a, .mean-container .mean-nav ul li a, #main-menu ul li.menu-item-simple-parent ul li a, .megamenu-child-container > ul.sub-menu > li > a, .megamenu-child-container > ul.sub-menu > li > .nolink-menu { ";
	...
	if (empty($disable_footer)) :
		...
		if (!empty($footer_content_font) || (!empty($footer_content_font_color) and $footer_content_font_color != "#") || !empty($footer_content_font_size)) :
			$output .= "#footer, #footer .widget.widget_recent_entries .entry-metadata .author, #footer .widget.widget_recent_entries .entry-meta .date, #footer label, #footer .widget ul li, #footer .widget ul li:hover, .copyright, #footer .widget.widget_recent_entries .entry-metadata .tags, #footer .categories {";
		...
		if(is_single()){
			...
			if (is_attachment()){
				...
				if($postType == 'post')	{
					...
					echo "<a href='{$link}'>{$title}</a>".$markup;
...
function dttheme_color_picker(){
	...
	foreach(getFolders(get_template_directory()."/skins") as $skin ):
		...
		$colors .= '<img src="'.$img.'" alt="color-'.$skin.'" title="color-'.$skin.'" width="30" height="30"/>';
		$colors .= '</a>';
		$colors .= '</li>';
	endforeach;
	...
	$str .= '	<a href="" title="" class="style-picker-ico"> <img src="'.IAMD_BASE_URL.'images/style-picker/picker-icon.png" alt="" title="" width="57" height="66" /> </a>';
