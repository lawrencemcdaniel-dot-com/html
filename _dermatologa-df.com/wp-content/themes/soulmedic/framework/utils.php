...
function dttheme_social_bookmarks($arg = 'sb-post') {
	...
	$fb = dttheme_option ( 'integration', "{$arg}-fb_like" );
	$data .= ! empty ( $fb ) ? "<li><a href='//facebook.com/sharer.php?u=$url&amp;t=".urlencode( $title )."'>
	...
	$stumbleupon = dttheme_option ( 'integration', "{$arg}-stumbleupon" );
	$data .= ! empty ( $stumbleupon ) ? "<li><a href='//stumbleupon.com/submit?url=$url&amp;title=".urlencode ( $title )."'>
	...
	$linkedin = dttheme_option ( 'integration', "{$arg}-linkedin" );
	$data .= ! empty ( $linkedin ) ? "<li><a href='http://linkedin.com/shareArticle?mini=true&amp;title=".urlencode ( $title )."&amp;url=$url' title='Share on LinkedIn'>
...
function dttheme_mailchimp_list_ids($apiKey) {
	...
	$url = '//' . $dataCenter . '.api.mailchimp.com/3.0/lists/';
...
function dt_mailchimp_check_member_already_registered($data, $apiKey, $listId) {
	...
	$dataCenter = substr($apiKey,strpos($apiKey,'-')+1);
	$url = '//' . $dataCenter . '.api.mailchimp.com/3.0/lists/' . $listId . '/members';
...
function dt_mailchimp_register_member($data, $apiKey, $listId) {
	...
	$memberId = md5(strtolower($data['email']));
	$dataCenter = substr($apiKey,strpos($apiKey,'-')+1);
	$url = '//' . $dataCenter . '.api.mailchimp.com/3.0/lists/' . $listId . '/members/' . $memberId;
	...
	if($httpCode == 200) {
		$out = '<span style="color:green;">'.esc_html__('Success! Please check your inbox or spam folder.', 'dt_themes').'</span>';
	} else {
		$out = '<span style="color:red;"><b>'.$result_decode['title'].':</b> '.$result_decode['detail'].'</span>';
	}
